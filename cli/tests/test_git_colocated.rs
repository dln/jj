...
fn test_colocated_workspace_independent_heads() {
    ...
    test_env.jj_cmd_ok(&repo_path, &["workspace", "add", "../second"]);
...
fn test_colocated_workspace_add_at_root() {
    ...
    test_env.jj_cmd_ok(test_env.env_root(), &["git", "init", "--colocate", "repo"]);
    test_env.jj_cmd_ok(&repo_path, &["workspace", "add", "../second"]);
    test_env.jj_cmd_ok(&repo_path, &["workspace", "add", "../third"]);
...
fn test_colocated_workspace_fail_existing_git_worktree() {
    ...
    insta::assert_snapshot!(
        test_env.jj_cmd_failure(&repo_path, &["workspace", "add", "../second"]),
        @r#"
    Done importing changes from the underlying Git repo.
    ...
    Command::new("git")
        ...
        .assert()
        .success();
    let _ = test_env.jj_cmd_ok(&repo_path, &["workspace", "add", "../second"]);
}
